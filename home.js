...
let homeLanguage = 'fr', activeVial = 'nad', paused = reducedMotion.matches, sceneVisible = true, tiltX = 0, tiltY = 0, scrollFrame = 0, changeId = 0;
function updateSceneCopy() {
  const copy = HOME_COPY[homeLanguage], vial = VIALS[activeVial];
  document.getElementById('sceneDescription').textContent = `${copy[activeVial]} · 100 mg`;
  document.getElementById('sceneImage').alt = `${copy.vial} ${vial.name} 100 mg`;
...
document.querySelectorAll('[data-vial]').forEach(button => button.addEventListener('click', async () => {
  ...
  document.getElementById('sceneWord').classList.toggle('long-name', next === 'ghk');
