...
const TRAY_STR = {
  fr:{title:'Ma sélection informative',one:'référence',many:'références',add:'Ajouter',added:'Ajoutée',copy:'Copier',download:'Télécharger la fiche',wa:'Demande sur WhatsApp',clear:'Vider',note:'Sélection informative uniquement : aucune commande ni paiement n’est effectué depuis cette page.',copied:'Références copiées.'},
  en:{title:'My information selection',one:'reference',many:'references',add:'Add',added:'Added',copy:'Copy',download:'Download sheet',wa:'WhatsApp request',clear:'Clear',note:'Information selection only: no order or payment is made from this page.',copied:'References copied.'},
  pt:{title:'A minha seleção informativa',one:'referência',many:'referências',add:'Adicionar',added:'Adicionada',copy:'Copiar',download:'Descarregar ficha',wa:'Pedido por WhatsApp',clear:'Limpar',note:'Apenas seleção informativa: não é efetuada qualquer encomenda ou pagamento nesta página.',copied:'Referências copiadas.'},
  es:{title:'Mi selección informativa',one:'referencia',many:'referencias',add:'Añadir',added:'Añadida',copy:'Copiar',download:'Descargar ficha',wa:'Solicitud por WhatsApp',clear:'Vaciar',note:'Solo selección informativa: no se realiza ningún pedido ni pago desde esta página.',copied:'Referencias copiadas.'}
};
...
function toggleSelection(btn){
  ...
  const item = { name: row.dataset.name, dose: row.dataset.dose, price: row.dataset.price };
...
function renderSelection(){
  ...
  document.getElementById('trayItems').innerHTML = selection.map((x,i)=>
    `<span class="chip">${x.name} · ${x.dose}<button type="button" onclick="removeSelection(${i})">×</button></span>`
  ).join('');
...
function selectionText(){
  const l = currentLang();
  return TRAY_STR[l].title + '\n\n' + selection.map(x=>`- ${x.name} — ${x.dose} — ${x.price}`).join('\n');
}
